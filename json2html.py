...
class Json2Html:
    def convert(self, json="", table_attributes='border="1"', colgroup_attributes=None, clubbing=True, encode=False, escape=True):
        """
            Convert JSON to HTML Table format with optional colgroup attributes
        """
        self.table_init_markup = "<table %s>" % table_attributes
        self.colgroup_markup = self._build_colgroup_markup(colgroup_attributes)
    ...
    def _build_colgroup_markup(self, colgroup_attributes):
        """
            Build the colgroup markup if attributes are provided
        """
        if not colgroup_attributes:
            return ""
        return "<colgroup %s></colgroup>" % colgroup_attributes
    ...
    def convert_list(self, list_input):
        ...
        if column_headers is not None:
            ...
            converted_output += self.colgroup_markup
    ...
    def convert_object(self, json_input):
        ...
        if not json_input:
            return ""
        converted_output = self.table_init_markup + self.colgroup_markup + "<tr>"
